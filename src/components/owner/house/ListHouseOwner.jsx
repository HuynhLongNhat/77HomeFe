...
const ListHouseOwner = () => {
    ...
    let  {data}  = await axios.get("http://localhost:8080/api/v1/house");
     if (data && data.DT) {
       ...
       if (listHouseByOwner.length > 0) {
         setListHouse(listHouseByOwner);
         setFilteredHouses(listHouseByOwner);
       }
     }
  ...
  return (
    <Container className="mt-4">
      ...
      {/* Tiêu đề */}
      <h1 className="text-center mb-4">Danh sách nhà trọ của  {auth.fullName}</h1>
